...
public class Main {
    public static void main(String[] args) {
        ...
        int n;
        ListaEncadeadaOrdenada<String> biblioteca;

        while (sc.hasNext()) {
            biblioteca = new ListaEncadeadaOrdenada<>();

            do n = sc.nextInt(); while (n < 1 || n > 1000);

            for (int i = 0; i < n; i++) {
                String livro;
                do livro = sc.next(); while (livro.length() > 4);
                while (livro.length() < 4) livro = "0" + livro;
                biblioteca.add(livro);
            }

            for (int i = 0; i < biblioteca.size(); i++) System.out.println(biblioteca.get(i));
        }
    }
}
